# Описание проекта
Проект представляет из себя сервис по загрузке и хранению файлов.
Реализована возможность загружать файлы на хостинг, просматривать список своих файлов, а также скачивать их. \
Скачивание доступно как по одному файлу (достаточно указать его id или путь, куда он загружался), так и несколько (архив).


## Запуск проекта
### Вариант 1. Из контейнера
Запустить команду `docker-compose up -d`. Сервис будет доступен по адресу http://127.0.0.1:8000

### Вариант 2. Вручную
Пример переменных окружения находится в файле `.env.expamle`

Для установки проекта необходимо:
1. Установить виртуальное окружение: `python3 -m venv venv`
2. Установить зависимости: `pip install -r requirements.txt`
3. Запустить БД из docker: `docker-compose up -d`
4. Осуществить миграции моделей БД: `alembic upgrade head`.\
   Если миграций нет, то их нужно предварительно создать:
    ```angular2html
